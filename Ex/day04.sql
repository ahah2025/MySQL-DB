...
-- 그룹함수 --> count()
-- 직원의 수를 구하시오
select count(*) -- 전체직원의 갯수(row 갯수)
from employees;

select count(commission_pct) -- 커미션(수당)을 받은 직원의 수(null을 제외한 갯수)
from employees;
...
select  ifnull(commission_pct,0), -- commission_pct 값이 null이면 0.00 으로 표시
		commission_pct
from employees;
...
-- 한번에 그룹별로 합계를 구한다      
-- *Group by에 참여한 컬럼(department_id)이나 그룹함수만 올 수 있다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              
...
select  department_id,
		max(salary),
        first_name -- group by에 들어가 있지 않아서 출력 X
from employees
group by department_id;
...
-- 상태컬럼은 수당이 없으면(null이면) 0, 수당이 있으면 1 을 state 컬럼에 표시하세요

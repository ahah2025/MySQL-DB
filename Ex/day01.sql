...
select 문(조회)
*****************************************/
-- select문 주석(select절 + from 절)
select * from employees; -- 마침표
SELecT * FROM Employees; -- 대소문자 구분하지 않음

-- 테이블 전체 데이터 조회
select * from employees;   -- 직원 테이블
select * from locations;   -- 도시 테이블
select * from regions;     -- 지역 테이블
select * from countries;   -- 나라 테이블
...
-- employee_id , fisrt_name , 성(last_name)
...
select first_name, -- 직원의 이름
	   ...
	   phone_number, -- 전화번호
	   email, -- 이메일
	   hire_date -- 입사일 from 앞에는 콤마(,)가 없다
 from employees;
 
-- 컬럼명에 별명사용하기
-- 직원아이디, 이름, 월급을 출력하세요.
-- 단 직원아이디는 empNO, 이름은 "f-name", 월급은 "월 급" 으로 컬럼명을 출력하세요
select employee_id empNO, -- as 생략 가능하다(employee_id as empNO)
	   first_name 'f-name',
       salary '월 급'
from employees;
...
select 	first_name, last_name,
        ...
        concat(first_name, ' ' , last_name) as '전체 이름',
		concat(first_name, ' ' , last_name, ' 입사일은 ', hire_date , ' 입니다.') as 문장    -- 이아름 입사일은 2024-01-01 입니다.
from employees;
